...
class VideoProcessor:
    def __init__(self, rtsp_url, cctv_id, user_id=None):
        ...
        self.frame_buffer = collections.deque(maxlen=120)  # 30프레임 버퍼 (약 1초)
        self.keypoints_buffer = collections.deque(maxlen=120)  # 포즈 키포인트 버퍼 (약 2초)
        self.last_detection_time = 0
        self.continuous_detection_count = 0
        self.detection_cooldown = 45  # 60초 쿨다운
        ...
        self.monitoring_period = 5  # 감지 모니터링 기간 (3초)
        self.min_detections = 2  # 녹화 시작을 위한 최소 감지 횟수 
        ...
        self.max_detection_gap = 15.0   # 허용되는 최대 감지 간격 (초)
        self.min_detection_count = 2   # 영상 저장에 필요한 최소 감지 횟수
    ...
    async def process_frames(self):
        ...
        while self.is_running:
            ...
            # 100 프레임마다 이상행동 감지 (3fps 학습 데이터와 일치: 30fps/10=3fps)
            if frame_count % 10 == 0:
                # 이상행동 감지 모델 실행
                try:
                            ...
                            if confidence >= self.min_confidence:
                                ...
                                if should_evaluate or (current_time - self.first_anomaly_time >= self.monitoring_period):
                                    ...
                                    if len(self.pending_anomalies) >= self.min_detections:
                                        # 평균 신뢰도 계산
                                        avg_confidence = np.median([conf for _, _, conf in self.pending_anomalies])
                    ...
                    if "추출된 키포인트의 형태가 비정상" in error_msg:
                        ...
                        logger.error(f"[{self.cctv_id}] 키포인트 처리 오류: {error_msg}", exc_info=True)
                except RuntimeError as e:
                    logger.error(f"[{self.cctv_id}] 모델 실행 오류: {str(e)}", exc_info=True)
                except Exception as e:
                    logger.error(f"[{self.cctv_id}] 예상치 못한 오류: {str(e)}", exc_info=True)
    ...
    def get_representative_behavior(self):
        ...
        # 행동별 가중 점수 계산
        behavior_scores = {}
        max_count = max(stats["count"] for stats in self.detected_behaviors.values())
        ...
        for behavior, stats in self.detected_behaviors.items():
            ...
            count_score = stats["count"] / max_count if max_count > 0 else 0
